...
def test_portuguese_tax_rules():
    ...
    for i, test_text in enumerate(test_cases, 1):
        ...
        if result:
            print(f"✅ Test {i}: {result['rule']} -> {result['deadline'].strftime('%Y-%m-%d')}")
        else:
...
def test_working_days():
    ...
    for days in scenarios:
        ...
        print(f"✅ {days} working days from {start_date.strftime('%Y-%m-%d')} -> {result.strftime('%Y-%m-%d (%A)')}")
    print()
...
def test_agent_processing():
    ...
    for i, doc_text in enumerate(test_documents, 1):
        ...
        if 'deadline' in result:
            ...
            urgency = "🔴 URGENT" if days_until <= 7 else "🟡 WARNING" if days_until <= 30 else "🟢 NORMAL"
            print(f"   ✅ Deadline: {result['deadline'].strftime('%Y-%m-%d')} ({days_until} days) {urgency}")
...
def test_business_metrics():
    ...
    sample_results = [
        {'deadline': datetime(2025, 6, 30), 'rule': 'IVA quarterly', 'priority': 'high'},
        {'deadline': datetime(2025, 7, 31), 'rule': 'Modelo 22', 'priority': 'high'},
        {'deadline': datetime(2025, 6, 25), 'rule': 'SAF-T monthly', 'priority': 'medium'},
        {'deadline': datetime(2025, 6, 10), 'rule': 'DMR monthly', 'priority': 'medium'},
    ]
...
def main():
    ...
    try:
        ...
        print("🎉 ALL TESTS PASSED! The EY AI Deadline Manager Agent is ready for deployment.")
        print()
        print("🚀 Next Steps:")
        print("   1. Run 'python3 -m streamlit run streamlit_app.py' to start the web interface")
