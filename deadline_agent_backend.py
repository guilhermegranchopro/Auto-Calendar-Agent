...
class DeadlineManagerAgent:
    ...
    def extract_text_from_image(self, image_path_or_file, use_mock_ocr=True):
        ...
        try:
            if use_mock_ocr:
                ...
                filename = getattr(image_path_or_file, 'name', str(image_path_or_file)).lower()
    ...
    def extract_text_from_pdf(self, pdf_path_or_file):
        ...
        try:
            ...
            if len(extracted_text.strip()) < 50:
                filename = getattr(pdf_path_or_file, 'name', str(pdf_path_or_file)).lower()
    ...
    def add_working_days(self, start_date, num_days):
        ...
        while days_added < num_days:
            ...
            if current_date.weekday() < 5 and current_date not in self.portuguese_holidays:
                days_added += 1
    ...
    def apply_portuguese_tax_rules(self, text, reference_date=None):
        ...
        if 'modelo 22' in text_lower or ('irs' in text_lower and ('modelo' in text_lower or 'deadline' in text_lower)):
        ...
        if ('modelo 30' in text_lower or 
            'retenções na fonte' in text_lower or 
            'retencao na fonte' in text_lower or
            'retencao' in text_lower):
        ...
        if ('dmr' in text_lower or 
            'declaração mensal de remunerações' in text_lower or
            'declaracao mensal' in text_lower):
            next_month = ref.replace(day=1) + relativedelta(months=1)
    ...
    def process_file(self, file_path_or_object, reference_date=None):
        ...
        try:
            ...
            if hasattr(file_path_or_object, 'name'):
                ...
                file_type = file_path_or_object.type if hasattr(file_path_or_object, 'type') else None
            else:
