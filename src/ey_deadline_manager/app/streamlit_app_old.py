...
# Core functions from the notebook (simplified versions for UI elements only)
...
def agent_process(text, reference_date=None):
    """Enhanced agent that uses backend with selected AI model"""
    # Get AI model from session state, default to gemini-pro
    ai_model = getattr(st.session_state, 'ai_model', 'gemini-pro')
    
    # Use backend processing with selected AI model
...
def main():
    ...
    ai_model = st.sidebar.selectbox(
        ...
        index=1,  # Default to the newer model
        help="Choose which Gemini model to use for AI-powered deadline extraction"
    )
    
    # Display model info
    if ai_model == "gemini-2.0-flash-001":
        st.sidebar.info("🚀 Using Gemini 2.0 Flash (Latest) via LangChain")
    else:
        st.sidebar.info("🔧 Using Gemini Pro (Classic) via Google GenAI")
...
def process_document(uploaded_file, manual_text, reference_date):
    ...
    with st.spinner("🔍 Analyzing document..."):
        ...
        display_processing_result(
            result, text, uploaded_file.name if uploaded_file else "Manual Input"
        )
...
def display_processing_result(result, text, filename):
    ...
    if "deadline" in result:
        ...
        <div class="{card_class}">
            ...
            <p><strong>🔧 Method:</strong> {processing_method.replace('_', ' ').title()}</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        ai_model = getattr(st.session_state, 'ai_model', 'gemini-pro')
        st.error(f"❌ Could not extract deadline with {ai_model}: {error_msg}")
...
def show_batch_processing():
    ...
    if data_folder.exists():
        ...
        st.warning(
            "Data folder not found. Please ensure sample documents are available."
        )
...
def process_all_documents(files):
    """Process all documents in the Data folder using backend with AI model selection"""
    ...
    for i, file_path in enumerate(files):
        ...
        status_text.text(f"Processing: {file_path.name} with {ai_model}")

        try:
            # Use backend file processing with selected AI model
            result = backend_process_file(file_path, ai_model=ai_model)
    ...
    status_text.text(f"✅ Processing complete with {ai_model}!")
...
def process_uploaded_batch(uploaded_files):
    """Process uploaded batch of files using backend with AI model selection"""
    ...
    for i, uploaded_file in enumerate(uploaded_files):
        status_text.text(f"Processing: {uploaded_file.name} with {ai_model}")
        ...
        try:
            ...
            # Process with backend using selected AI model
            result = backend_process_text(text, ai_model=ai_model)
...
def create_analytics_charts(results):
    ...
    df["urgency"] = df["days_until"].apply(
        lambda x: "Urgent (≤7 days)"
        if x <= 7
        else "Warning (≤30 days)"
        if x <= 30
        else "Normal (>30 days)"
    )
    ...
    with col1:
        st.metric(
            ...
            f"{(time_saved_hours / total_docs * 60):.1f} min/doc",
        )
    ...
    with col2:
        st.metric(
            "💰 Cost Savings",
            f"€{cost_savings:.2f}",
            f"€{cost_savings / total_docs:.2f}/doc",
        )
...
def run_presentation_demo(demo_option):
    ...
    with st.spinner("🤖 Step 2: AI Agent Processing..."):
        ...
        # Get AI model from session state
        ai_model = getattr(st.session_state, 'ai_model', 'gemini-pro')
        result = backend_process_text(scenario["text"], ai_model=ai_model)
        st.success(f"✅ Deadline analysis complete using {ai_model}")
